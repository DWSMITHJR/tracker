...
class Program
{
    static void Main()
    {
        string outputFile = Path.Combine(Environment.CurrentDirectory, "dotnet_check.txt");
        
        using (StreamWriter writer = new StreamWriter(outputFile))
        {
            writer.WriteLine("=== Simple .NET Check ===");
            ...
            try
            {
                ...
                writer.WriteLine("File system write test: " + (fileWriteSuccess ? "SUCCESS" : "FAILED"));
            }
            catch (Exception ex)
            {
                writer.WriteLine("File system test failed: " + ex.Message);
            }
